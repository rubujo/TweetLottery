...
public partial class FMain
{
        ...
        bool notEmulateManualSurf = false)
    {
        ...
        foreach (TweetData tweetData in listTweetData.OrderBy(n => n.CreatedAt))
        {
            ...
            subItems[5] = string.IsNullOrEmpty(tweetData.IDStr) ? string.Empty : $"https://twitter.com/i/web/status/{subItems[2]}";
            subItems[6] = tweetData.UserData?.ProfileImageUrlHttps?.ToString() ?? string.Empty;
            subItems[7] = string.IsNullOrEmpty(screenName) ? string.Empty : $"https://twitter.com/{screenName}";
    ...
    public async Task DrawTweets(decimal drawAmount, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            string drawFininsedMessage = "推文抽取完成。";

            WriteLog(this, drawFininsedMessage);

            ShowMsg(this, drawFininsedMessage);
    ...
    public static void OpenTweetUrl(ListView listView, MouseEventArgs e)
    {
        listView.InvokeIfRequired(() =>
        {
            ...
            {
                if (focusedItem.SubItems.Count >= listView.Columns.Count)
                {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            string finishedMessage = $"推文獲取完成，共獲取到 {FetchedTweets.Count} 則推文。";
        ...
        catch (OperationCanceledException)
        {
            try
            {
                string canceledMessage = $"已取消作業，共獲取到 {FetchedTweets.Count} 則推文。";
        ...
        try
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    double[] widthSet = { 5.0, 30.0, 30.0, 30.0, 30.0, 80.0, 30.0, 60.0, 60.0, 60.0 };
                    ...
                    workbook.Properties.Comments = "由推文抽獎應用程式匯出。";
                    workbook.Properties.Company = string.Empty;

                    package.SaveAs(fileStream);

                    ShowMsg(form, $"已匯出{fileName}。");
                }
                catch (Exception ex)
                {
